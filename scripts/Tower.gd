...
func _spawn_projectile(target: Node2D) -> void:
	...
	projectile.global_position = global_position + Vector2(0.0, -26.0)
...
func _play_hit_feedback() -> void:
	...
	_hit_flash_tween.tween_property(self, "modulate", Color(1, 1, 1, 1), 0.12)

func _update_team_visuals() -> void:
	var banner := get_node_or_null("TeamBanner") as Polygon2D
	if banner == null:
		return
	banner.color = Color(0.2, 0.6, 1.0, 1.0) if team == 0 else Color(1.0, 0.35, 0.35, 1.0)
